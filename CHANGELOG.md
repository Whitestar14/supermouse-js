...
- **Priority Scheduling:** Plugins now support a `priority` property.
  - Negative priority (e.g. `-10`): **Logic Plugins** (Magnetic, Stick) run _before_ physics to modify the target.
  - Positive priority (e.g. `0`): **Visual Plugins** run _after_ physics to render the state.
