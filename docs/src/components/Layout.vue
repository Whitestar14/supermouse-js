...
// Allow child components (like the Playground Editor) to request the global cursor be paused
// so they can render their own cursor without conflict.
provide('setGlobalCursorState', (enabled: boolean) => {
  if (enabled) {
    if (!mouse) initCursor();
    else mouse.enable();
  } else {
