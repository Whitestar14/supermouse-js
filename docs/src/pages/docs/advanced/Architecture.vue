  ...
  <DocsSection label="Advanced" title="Core Architecture">
    <div class="mb-16">
      <p class="text-lg text-zinc-600 leading-relaxed mb-6">
        ...
        and
        <span class="text-black font-bold border-b-2 border-black/10"
          >Visual Rendering</span
        >.
      </p>
      <p class="text-lg text-zinc-600 leading-relaxed">
        This separation is what allows a magnetic button to "pull" the cursor
        (modifying Intent) without the input system needing to know, and without
        the visual dot jumping instantly (Physics handles the smoothing).
      </p>
    </div>
    ...
    <div class="relative border-l-2 border-zinc-200 ml-4 md:ml-8 space-y-16">
      ...
      <TimelineStep phase="Phase 01: Sense" title="Input Normalization">
        <Text>
          The <code>Input</code> system captures native DOM events (mousemove,
          touchmove). It normalizes coordinates relative to the container and
          checks for accessibility preferences (reduced-motion).
        </Text>
      ...
      <TimelineStep phase="Phase 02: Intent" title="Logic Plugins">
        <Text>
          Plugins with <code>priority &lt; 0</code> execute. This is where
          "magic" happens. A magnetic plugin might look at the hovered element
          and say "Hey, I know the mouse is at 500, but I want the cursor to
          snap to the center of this button at 550."
        </Text>
        <ResponsibilityBox title="Responsibility">
      ...
      >
        <Text>
          The Core calculates the difference between where we are (<span
            ...
            >smooth</span
          >) and where we want to go (<span
            class="mono text-xs text-black bg-zinc-100 px-1"
            >target</span
          >). It applies frame-rate independent damping to calculate the new
          position for this specific frame.
        </Text>
        <CodeBlock
          :code="dampCode"
          lang="javascript"
      ...
      >
        <Text>
          ...
          <code>state.smooth</code> coordinates and apply CSS transforms to
          their DOM elements.
        </Text>
        <ResponsibilityBox title="Responsibility">
    ...
    <div
      class="grid grid-cols-1 md:grid-cols-2 gap-px bg-zinc-200 border border-zinc-200"
    >
      <div class="bg-white p-8">
        ...
        <p class="text-sm text-zinc-600 leading-relaxed mb-4">
          ...
          (<code>getComputedStyle</code>) during the render loop causes layout
          thrashing.
        </p>
        <p class="text-sm text-zinc-600 leading-relaxed">
          Supermouse solves this by parsing interaction data
          <strong>only once</strong> when the <code>mouseover</code> event
          fires. It stores the result in a <code>WeakMap</code> cache. The loop
          reads from this cache, making interaction checks O(1).
        </p>
      </div>
      <div class="bg-white p-8">
        <h4 class="text-lg font-bold text-zinc-900 mb-4">The Stage System</h4>
        <p class="text-sm text-zinc-600 leading-relaxed mb-4">
          The "Double Cursor" glitch happens when an element's CSS (`cursor:
          pointer`) overrides the body's `cursor: none`.
        </p>
        <p class="text-sm text-zinc-600 leading-relaxed">
          Hiding the body cursor won't be enough, so Supermouse injects a
          dynamic <code>&lt;style&gt;</code> tag that targets every interactive
          selector registered by plugins (e.g.
          <code>a, button, [data-hover]</code>) and applies
    ...
    <div class="mt-16">
      <h3
        class="text-sm font-bold text-zinc-900 mb-4 uppercase tracking-widest"
      >
