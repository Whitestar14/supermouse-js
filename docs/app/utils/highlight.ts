...
function highlightVue(code: string): string {
  ...
  const scriptContent =
    scriptMatch && scriptMatch[2] ? highlightCore(scriptMatch[2], JS_GRAMMAR) : "";
  const templateContent =
    templateMatch && templateMatch[2] ? highlightCore(templateMatch[2], HTML_GRAMMAR) : "";
  const styleContent =
    styleMatch && styleMatch[2] ? highlightCore(styleMatch[2], CSS_GRAMMAR) : "";
